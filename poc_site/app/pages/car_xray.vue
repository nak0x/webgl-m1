<template>
  ...
  <!-- Tooltip hover -->
  <div v-if="hoveredPart" class="xray-tooltip">
    {{ hoveredPart }}
  </div>
  ...
  <aside class="xray-panel xray-panel--right">
    ...
    <div
      ...
      :key="part.name"
      class="xray-part"
      :class="{ 'xray-part--hovered': hoveredPart === part.name }"
    >
    ...
    <div v-if="debugOpen" class="xray-debug">
      <div v-for="ctrl in shaderControls" :key="ctrl.key" class="xray-slider">
        <div class="xray-slider__header">
          ...
          <span class="xray-slider__value">{{ shader[ctrl.key].toFixed(2) }}</span>
        </div>
        <input
          type="range"
          :min="ctrl.min"
...
const shader = reactive({ ...XRAY_DEFAULTS })
...
function onShaderInput(key, e) {
  shader[key] = parseFloat(e.target.value)
  world?.setShaderParams(shader)
}

function resetShader() {
  Object.assign(shader, XRAY_DEFAULTS)
  world?.setShaderParams(shader)
}
...
function setDamage(partName, level) {
  world?.setPartDamage(partName, level)
  const p = parts.value.find(p => p.name === partName)
  if (p) p.damageLevel = level
}

onMounted(() => {
  experience = new Experience(canvas.value, CarXraySources)

  world = new CarXrayWorld(experience, {
    onPartsReady: (list) => { parts.value = list },
    onHighlight:  (name) => { hoveredPart.value = name },
  })
